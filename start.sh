...
function setup {
    ...
    sudo tc qdisc add dev lo parent 1:3 handle 30: netem delay 9ms 1ms distribution normal
    ...
    sudo iptables -t filter -A INPUT  -s 127.2.0.0/16 -d 127.2.0.0/16 -j MULTISITE
    sudo iptables -t filter -A OUTPUT -s 127.2.0.0/16 -d 127.2.0.0/16 -j MULTISITE
    sudo iptables -t filter -A INPUT  -s 127.3.0.0/16 -d 127.3.0.0/16 -j MULTISITE
    sudo iptables -t filter -A OUTPUT -s 127.3.0.0/16 -d 127.3.0.0/16 -j MULTISITE

    # Nothing goes from 127.2._._ to 127.3._._ => Hzct 2 and 3 are contained
    sudo iptables -t filter -A INPUT  -s 127.2.0.0/16 -d 127.3.0.0/16 -j DROP
    sudo iptables -t filter -A INPUT  -s 127.3.0.0/16 -d 127.2.0.0/16 -j DROP
    sudo iptables -t filter -A OUTPUT -s 127.2.0.0/16 -d 127.3.0.0/16 -j DROP
    sudo iptables -t filter -A OUTPUT -s 127.3.0.0/16 -d 127.2.0.0/16 -j DROP
...
export -f status2 shut2 open2 split2 unsplit2
export -f status3 shut3 open3 split3 unsplit3
export -f setup tear

echo "undoing previously setup things if any"
tear
echo "setting up the new things"
setup
...
echo ""
echo "split2  : nodes 3 and 4 are unreachable"
echo ""
